                          ...
                          uint8_t m2[MLEN + CRYPTO_BYTES], uint8_t ctx[CTXLEN])
{
  ...
  CHECK(mld_sign_keypair(pk, sk, NULL) == 0);
  CHECK(randombytes(ctx, CTXLEN) == 0);
  MLD_CT_TESTING_SECRET(ctx, CTXLEN);
  CHECK(randombytes(m, MLEN) == 0);
  MLD_CT_TESTING_SECRET(m, MLEN);

  CHECK(mld_sign(sm, &smlen, m, MLEN, ctx, CTXLEN, sk, NULL) == 0);

  rc = mld_sign_open(m2, &mlen, sm, smlen, ctx, CTXLEN, pk, NULL);
...
static int test_sign_extmu(void)
{
  ...
  CHECK(mld_sign_signature_extmu(sig, &siglen, mu, sk, NULL) == 0);
  CHECK(mld_sign_verify_extmu(sig, siglen, mu, pk, NULL) == 0);
...
static int test_sign_pre_hash(void)
{
  ...
  CHECK(mld_sign_signature_pre_hash_shake256(sig, &siglen, m, MLEN, ctx, CTXLEN,
                                             rnd, sk, NULL) == 0);
  CHECK(mld_sign_verify_pre_hash_shake256(sig, siglen, m, MLEN, ctx, CTXLEN, pk,
                                          NULL) == 0);
...
static int test_pk_from_sk(void)
{
  ...
  CHECK(mld_sign_pk_from_sk(pk_derived, sk, NULL) == 0);
  ...
  rc = mld_sign_pk_from_sk(pk_derived, sk_corrupted, NULL);
